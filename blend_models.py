...
def run(low_res_pkl: Path, # Pickle file from which to take low res layers
...
def main():
    parser = argparse.ArgumentParser(
        description='Blend two models.'
    )
    parser.add_argument('--low_res_pkl', help='Pickle file from which to take low res layers', required=True, type=Path)
    parser.add_argument('--high_res_pkl', help='Pickle file from which to take high res layers', required=True, type=Path)
    parser.add_argument('--resolution', help='Resolution level at which to switch between models', required=True, type=int)
    parser.add_argument('--level', help='Switch at Conv block 0 or 1?', type=int, default=0)
    parser.add_argument('--blend_width', help='None = hard switch, float = smooth switch (logistic) with given width', type=Optional[float], default=None)
    parser.add_argument('--output_grid', help=' Path of image file to save example grid (None = dont save)', type=Path, default='blended.jpg')
    parser.add_argument('--seed', help='seed for random grid', type=int, default=0)
    parser.add_argument('--output_pkl', help='Output path of pickle (None = dont save)', type=Path, default=None)
    parser.add_argument('--verbose', help='Print out the exact blending fraction', type=bool, default=False)
